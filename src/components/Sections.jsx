...
import { formatPrice, getExtendedContent, getUi, googleRating, pricingData, trustBar } from '../content/site';
...
export function TestimonialSection({ language, testimonials, compact = false, serviceType }) {
  ...
  return <section className={`section testimonial-section ${compact ? 'testimonial-compact' : ''}`}><div className="container"><div className="testimonial-heading"><div><span className="section-label">{copy.testimonialLabel}</span><h2>{copy.testimonialsTitle || copy.testimonialHeading}</h2><p className="testimonial-intro">{copy.testimonialsIntro}</p></div>{visible.length > 1 && <div className="carousel-controls"><button type="button" className="is-prev" aria-label={copy.carouselPrev} onClick={() => scroll(-1)}><Icon name="arrow" size={18} /></button><button type="button" aria-label={copy.carouselNext} onClick={() => scroll(1)}><Icon name="arrow" size={18} /></button></div>}</div><div className="testimonial-track" ref={trackRef}>{visible.map((item) => <blockquote className="testimonial-card" key={item.id || item.name}><span className="quote-mark" aria-hidden="true">“</span><p>{item.quote}</p><footer><div className="testimonial-person"><span className="testimonial-avatar"><span className="avatar-fallback" aria-hidden="true">{item.name.slice(0, 1)}</span>{item.avatarUrl && <img src={item.avatarUrl} alt="" onError={(event) => event.currentTarget.remove()} />}</span><div><strong>{item.name}</strong><span>{item.context}</span></div></div>{item.status === 'preview' && <small>{copy.exampleTestimonial}</small>}</footer></blockquote>)}</div>{visible.length === 0 && <p className="testimonial-note">{copy.noTestimonials || ''}</p>}<p className="testimonial-note">{getExtendedContent(language).ratingNote}</p></div></section>;
}

const STAR_PATH = 'm12 3 2.78 5.63 6.22.9-4.5 4.39 1.06 6.2L12 17.2l-5.56 2.92 1.06-6.2L3 9.53l6.22-.9L12 3Z';

export function HeroMedia({ image }) {
  return <><div className="detail-hero-media" style={{ backgroundImage: `url(${image})` }} aria-hidden="true" /><div className="detail-hero-scrim" aria-hidden="true" /></>;
}

const GOOGLE_LETTERS = [['G', '#4285f4'], ['o', '#ea4335'], ['o', '#fbbc05'], ['g', '#4285f4'], ['l', '#34a853'], ['e', '#ea4335']];

function GoogleMark({ size }) {
  return <span className="google-mark" style={size ? { fontSize: size } : undefined} aria-hidden="true">{GOOGLE_LETTERS.map(([letter, color], index) => <span key={index} style={{ color }}>{letter}</span>)}</span>;
}

function Stars({ rating }) {
  return <div className="stars" aria-hidden="true">{[0, 1, 2, 3, 4].map((index) => {
    const fill = Math.min(1, Math.max(0, rating - index));
    return <span className="star" key={index}><svg className="star-base" viewBox="0 0 24 24"><path d={STAR_PATH} /></svg><svg className="star-fill" style={{ clipPath: `inset(0 ${(1 - fill) * 100}% 0 0)` }} viewBox="0 0 24 24"><path d={STAR_PATH} /></svg></span>;
  })}</div>;
}

export function TrustBar({ language = 'en' }) {
  const headline = language === 'en' ? trustBar.headline.en : trustBar.headline.zh;
  const showRating = googleRating.status !== 'preview' && googleRating.status !== 'hidden';
  if (!headline && !trustBar.licence && !showRating) return null;
  return <section className="trust-bar"><div className="container">
    {headline && <p className="trust-bar-headline">{headline}</p>}
    {showRating && <div className="trust-bar-score"><GoogleMark /><Stars rating={googleRating.rating} /><strong>{googleRating.rating.toFixed(1)}/5.0</strong></div>}
    {trustBar.licence && <p className="trust-bar-licence">{trustBar.licence}</p>}
  </div></section>;
}
...
export function GoogleRatingSection({ language }) {
  ...
  return <section className="section google-rating-section"><div className="container google-rating-layout"><div className="google-score"><GoogleMark /><strong>{googleRating.rating.toFixed(1)} / 5</strong><span className="sr-only">{`${googleRating.rating.toFixed(1)} out of 5`}</span><div className="star-row" aria-hidden="true"><Stars rating={googleRating.rating} /></div></div><div><span className="section-label">{copy.googlePreviewKicker}</span><h2>{copy.googlePreviewTitle}</h2><p>{copy.googlePreviewText}</p><strong className="google-preview-note">{copy.googlePreviewScoreNote}</strong>{preview && <p className="google-link-note">{copy.googlePreviewNoLink}</p>}{!preview && googleRating.profileUrl && <a className="inline-link" href={googleRating.profileUrl} target="_blank" rel="noreferrer">{copy.googleViewReviews} <Icon name="arrow" size={15} /></a>}</div></div></section>;
}
...
export function RegistrationInquiry({ language, kind = 'local', needsAddress = false, setNeedsAddress = () => {} }) {
  ...
  const submit = async (event) => { event.preventDefault(); const formEl = event.currentTarget; setStatus('sending'); try { await submitEnquiry({ ...form, type: 'company-registration', service: copy.registrationInquiry, needsAddress: needsAddress ? copy.yes : copy.no, consent: form.consent, website: form.website }); setStatus('success'); formEl.reset(); setForm({ profile: kind === 'mainland' ? copy.mainland : copy.local }); } catch { setStatus('error'); } };
