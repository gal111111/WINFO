...
const finderCopy = {
  ...
  'zh-CN': { title: '不确定应该从哪里开始？', body: '选择最接近你情况的一项，我们会带你到最有用的第一个对话。', stepOne: '先告诉我们你现在处于哪一步', stepTwo: '建议从这里开始', back: '重新选择', options: ['我准备在香港开始', '我需要处理公司合规', '我想拓展跨境业务', '我正在规划 IANG 落地'], result: '建议从这里开始', cta: '查看推荐服务' },
};
...
const copy = {
  ...
  'zh-CN': {
    ...
    menu: { start: '在香港开始事业', compliance: '稳妥处理合规', grow: '拓展跨境业务', explore: '查看全部服务', support: '不确定应该从哪里开始？', talk: '与顾问对话' },
    brand: 'WINFO 盈丰商业有限公司',
    hero: { title: '在香港开始、经营，\n再把事业拓展出去。', body: '清晰建议，务实执行。从公司成立、合规管理到跨境发展，由同一支可靠团队陪你前进。', primary: '预约咨询', secondary: '查看服务', note: '为创业者、运营团队及跨境企业而设。', visualLabel: '香港 / 商业运营', visualTitle: '让下一步更清楚。', visualText: '从首次注册到下一个市场，我们把复杂环节整理成可执行的步骤。' },
    proof: ['公司成立', '会计与审计', '资助与基金', 'IANG 一条龙服务'],
    home: { servicesKicker: '我们的服务', servicesTitle: '每个阶段，\n都有合适的支持。', servicesBody: '按你下一个决定选择服务。我们让范围清楚、流程透明、建议务实。', viewAll: '查看全部服务', whyKicker: '为什么选择 WINFO', whyTitle: '做好决定，\n不只是列出清单。', whyBody: '我们结合香港运营经验与跨境执行能力，让你不用靠猜，也能有把握地前进。', principles: ['开始前先确认清晰范围', '团队可以直接采取的实际步骤', '由同一位负责人跟进'], casesKicker: '精选成果', casesTitle: '围绕真正的商业决定，\n建立清晰的工作方法。', casesBody: '从进入市场、申请资助，到持续合规和有序扩张，为每个关键决定建立清晰框架。', casesButton: '查看案例', quote: 'WINFO 把复杂的香港落地工作，整理成团队真正可以掌握的一连串决定。', quoteBy: '跨境消费品牌创始人', finalTitle: '准备好，\n让下一步更清楚吗？', finalBody: '告诉我们你现在的情况，我们一起整理最有用的下一步。', finalButton: '开始对话' },
    services: { kicker: '服务范围', title: '让企业支持更容易理解，\n也更容易执行。', body: '为创业者和跨境团队准备的完整服务框架。你可以从一项需要开始，也可以把整体情况交给我们。', featured: '重点服务', all: '全部服务', path: '查看服务', iangTitle: '香港 IANG 一条龙服务', iangBody: '为毕业生和专业人士准备的香港落地方案，支持你在香港开展下一个阶段。', iangTag: '新增服务' },
    ...
    cases: { kicker: '案例', title: '成果背后，\n是一套清楚的工作方法。', body: '用这个页面整理起点、关键决定和最终成果，让潜在客户更快理解你的专业价值。', filters: ['全部', '起步', '合规', '拓展'], projects: [{ type: '起步', title: '从第一个想法到香港运营基地', body: '为首次进入香港的内地创业者，整理一套清晰的落地步骤。', result: '启动顺序更清楚' }, { type: '合规', title: '为成长中的团队建立合规节奏', body: '以实际年度框架整理定期申报和财务工作，减少临急决定。', result: '减少最后一刻的工作' }, { type: '拓展', title: '把资助机会变成可执行计划', body: '为准备下一阶段的企业整理申请条件、执行路线和市场拓展方向。', result: '团队可以直接行动' }] },
    approach: { kicker: '我们的方式', title: '先把事情说清楚，\n再让它向前走。', body: '让复杂工作更容易理解、更容易分工，也更容易持续推进。', steps: [{ n: '01', title: '了解', body: '先整理你的情况、限制，以及你真正需要作出的决定。' }, { n: '02', title: '组织', body: '把各个环节整理成清晰范围、次序和责任。' }, { n: '03', title: '执行', body: '协调实际工作，在重要节点让你掌握进度。' }, { n: '04', title: '延伸', body: '留下更稳定的运营节奏，支持下一个阶段。' }] },
    about: { kicker: '关于 WINFO', title: '陪伴认真建设事业的人，\n稳定走好每一步。', body: 'WINFO 盈丰商业有限公司结合香港企业服务、金融视角和跨境执行能力，协助认真建设事业的人作出清楚决定，再把决定落实成实际行动。', stats: [{ value: 'HK', label: '立足香港' }, { value: '01', label: '专责团队' }, { value: '360°', label: '整体视角' }], valuesTitle: '我们如何与你合作', values: ['清楚说明已知事项，也标明仍需确认的部分。', '把建议连接到你眼前真正要作出的决定。', '在完成当前工作前，先让下一步变得可见。'] },
    contact: { kicker: '联系我们', title: '告诉我们，\n你正在建设什么。', body: '简单说说你的情况，我们会回复你一个清晰可行的起点。', form: { name: '你的姓名', email: '工作邮箱', company: '公司名称', message: '你希望我们协助什么？', send: '提交咨询', success: '谢谢，你的咨询已准备好跟进。' }, detailsTitle: 'WINFO 盈丰商业有限公司', address: '香港九龙观塘敬业街 61-63 号利维大厦 1 楼 122 室', email: 'enquiry@winfo.hk', phone: '+852 6651 5286', hours: '星期一至五，上午 9 时至下午 6 时' },
    ...
    footer: { line: '为在香港及更远地方建设事业的人而设。', services: '服务', company: '公司', legal: '法律信息', privacy: '隐私政策', copyright: 'WINFO 盈丰商业有限公司。版权所有。' },
  },
};
...
function PageIntro({ kicker, title, body }) { return <section className="page-intro"><div className="content-width page-intro-inner"><div className="eyebrow"><span className="eyebrow-dot" />{kicker}</div><h1>{title}</h1><p>{body}</p></div></section>; }
...
function CasesPage({ t, lang, navigate }) { const [filterIndex, setFilterIndex] = useState(0); const activeFilter = t.cases.filters[filterIndex] || t.cases.filters[0]; const projects = filterIndex === 0 ? t.cases.projects : t.cases.projects.filter((item) => item.type === activeFilter); return <PageShell className="cases-page"><PageIntro kicker={t.cases.kicker} title={t.cases.title} body={t.cases.body} /><section className="section cases-section"><div className="content-width"><div className="filter-row" role="tablist" aria-label={t.cases.kicker}>{t.cases.filters.map((item, index) => <button type="button" role="tab" aria-selected={filterIndex === index} className={filterIndex === index ? 'is-active' : ''} key={item} onClick={() => setFilterIndex(index)}>{item}</button>)}</div><div className="case-list" aria-live="polite">{projects.map((project, index) => <article className="case-row" key={project.title}><span className="case-row-number">{String(index + 1).padStart(2, '0')}</span><div><span className="case-kicker">{project.type}</span><h2>{project.title}</h2><p>{project.body}</p></div><div className="case-result"><span>RESULT</span><strong>{project.result}</strong></div></article>)}</div></div></section><TestimonialSection lang={lang} page="cases" /><PageCta t={t} navigate={navigate} /></PageShell>; }

function ApproachPage({ t, lang, navigate }) { return <PageShell className="approach-page"><PageIntro kicker={t.approach.kicker} title={t.approach.title} body={t.approach.body} /><section className="section approach-section"><div className="content-width approach-list">{t.approach.steps.map((step) => <article className="approach-step" key={step.n}><span className="step-number">{step.n}</span><div><h2>{step.title}</h2><p>{step.body}</p></div><span className="step-arrow">↘</span></article>)}</div></section><TestimonialSection lang={lang} page="approach" /><PageCta t={t} navigate={navigate} /></PageShell>; }
